...
* Version      : 1.00
* Description  : This module implements the Flash API control commands for the RE01 Group(256KB products)
********************************************************************************************************************/
/***********************************************************************************************************************
* History : DD.MM.YYYY Version Description
*         : 16.12.2019 0.4     First Release (Ported from 1500KB)
*                              - Change the include file names
*                              - Due to change UMH, change the resister name
*                                1) Old:FLASH->FLWT                      New:SYSF->FLWT
*                                2) Old:FLASH->FWEPROR                   New:SYSC->FWEPROR
*                                3) Old:FLASH->xxxx (other than 1),2))   New:FACI->xxxx
*                              - Change the constant name
*                              - Update comments
*         : 16.1.2020  0.8     Remove the Conditional Constructs about RE01 1500KB WS1 chip
...
#define FLASH_FCU_INT_ENABLE    FACI->FAEINT = 0x90;   \
                                FACI->FRDYIE = 0x01;   ///< Enable FCU interrupts
#define FLASH_FCU_INT_DISABLE   FACI->FAEINT = 0x00;   \
                                FACI->FRDYIE = 0x00;   ///< Disable FCU interrupts

#define FACI_8   ((FLASHACI_Type8*)  FACI_COMMAND)
...
static flash_err_t flash_init ()
{
    ...
    SYSC->FWEPROR = FCU_FWEPROR_FLWE_PERMIT;
...
static flash_err_t flash_clock_config (uint32_t cur_fclk)
{
    ...
    FACI->FPCKAR = (uint16_t)(FCU_FPCKAR_KEY) + (uint16_t)speed_mhz;
...
static flash_err_t flash_pe_mode_enter (flash_type_t flash_type)
{
    ...
    if (flash_type == FLASH_TYPE_DATA_FLASH)
    {
        ...
        FACI->FENTRYR = FCU_FENTRYR_KEY | FCU_FENTRYR_DATA;        //Transition to DF P/E mode
        if (FACI->FENTRYR == FCU_FENTRYR_DATA)
        {
            err = FLASH_SUCCESS;
        }
        else
    ...
    {
        FACI->FENTRYR = FCU_FENTRYR_KEY | FCU_FENTRYR_CODE;        //Transition to CF P/E mode
        while ((FCU_FENTRYR_CODE != FACI->FENTRYR) && (count++ < FLASH_FENTRYR_TIMEOUT))
        {
            /* Do nothing */
        }

        if (FCU_FENTRYR_CODE == FACI->FENTRYR)
        {
            err = FLASH_SUCCESS;
        }
        else
...
static flash_err_t flash_pe_mode_exit ()
{
    ...
    if (0 != FACI->FASTAT_b.CMDLK)
    {
        /*Issue a Status Clear to clear Command Locked state*/
        FACI_8->COM8 = (uint8_t) FCU_FACI_STATUS_CLEAR_CMD;
        temp_err = FLASH_ERR_CMD_LOCKED;
    }

    /*Transition to Read mode*/
    FACI->FENTRYR = FCU_FENTRYR_KEY | FCU_FENTRYR_READ;
    while ((FCU_FENTRYR_READ != FACI->FENTRYR) && (count++ < FLASH_FENTRYR_TIMEOUT))
    {
        /* Do nothing */
    }

    if (FCU_FENTRYR_READ == FACI->FENTRYR)
    {
        err = FLASH_SUCCESS;
    }
    else
...
static flash_err_t flash_stop ()
{
    ...
    FACI_8->COM8 = (uint8_t) FCU_FACI_FORCED_STOP_CMD;
...
static flash_err_t flash_erase (uint32_t block_address, uint32_t num_blocks)
{
            ...
            g_current_parameters.current_count++)
    {
        ...
        FACI->FSADDR = g_current_parameters.dest_addr;
        
        /*Issue two part Block Erase commands*/
        FACI_8->COM8 = FCU_FACI_BLOCK_ERASE_CMD1;
        FACI_8->COM8 = FCU_FACI_BLOCK_ERASE_CMD2;
        ...
        if (0 != FACI->FASTAT_b.CMDLK)
        {
            if ((FACI->FSTATR_b.ILGCOMERR) || (FACI->FSTATR_b.ILGLERR))
            {
                err = FLASH_ERR_ACCESSW;
            }
            else if (FACI->FSTATR_b.SECERR)
            {
                err = FLASH_ERR_SECURITY;
            }
            else if ((FACI->FSTATR_b.PRGERR) || (FACI->FSTATR_b.ERSERR))
            {
                err = FLASH_ERR_FAILURE;
            }
            else
        ...
        if (FCU_FENTRYR_DATA == FACI->FENTRYR)
        {
        ...
        else if (FCU_FENTRYR_CODE == FACI->FENTRYR)
        {
        ...
        flash_program_mode_t pgm_mode)
{
    ...
    while (g_current_parameters.total_count > 0)
    {
        ...
        FACI_8->COM8 = FCU_FACI_PROGRAM_CMD1;
        FACI_8->COM8 = g_current_parameters.min_pgm_size;
        ...
        FACI_8->COM8 = FCU_FACI_PROGRAM_CMD2;
...
static flash_err_t flash_reset ()
{
    ...
    if (FACI->FASTAT_b.CFAE == 1)
    {
        FACI->FASTAT_b.CFAE = 0;
    }
#ifndef FLASH_NO_DATA_FLASH
    if (FACI->FASTAT_b.DFAE == 1)
    {
        FACI->FASTAT_b.DFAE = 0;
    }
#endif
...
static void flash_set_current_swap_state (uint8_t value)
{
    ...
    {
        if (FLASH_SAS_SWAPFLG == FACI->FSUACR_b.SAS)
        {
            if (1 == FACI->FAWMON_b.BTFLG)     // 1 = default
            {
        ...
        {
            if (FLASH_SAS_ALTERNATE == FACI->FSUACR_b.SAS)
            {
                sas_flag = FLASH_SAS_DEFAULT;
            }
            else
    ...
    FACI->FSUACR = reg_value;

    while (sas_flag != FACI->FSUACR_b.SAS)
    {
...
static flash_err_t flash_get_access_window (flash_access_window_config_t *pAccessInfo)
{
    pAccessInfo->start_addr = (FACI->FAWMON_b.FAWS << 12);
    pAccessInfo->end_addr = (FACI->FAWMON_b.FAWE << 12);
...
static flash_err_t flash_write_faw_reg (fawreg_t faw)
{
    ...
    FACI->FSADDR_b.FSADDR = FCU_CONFIG_REG_ADDR;
    FACI_8->COM8 = (uint8_t)FCU_FACI_CONFIG_CMD1;
    FACI_8->COM8 = (uint8_t)FCU_FACI_CONFIG_CMD2;
    ...
    FACI_8->COM8 = (uint8_t)FCU_FACI_CONFIG_CMD3;
...
static void flash_ready_isr (void)
{
    ...
    {
        /* Resume P/E */
        FACI_8->COM8 = (uint8_t) FCU_FACI_PROGRAM_CMD2;
        ...
        if (g_current_parameters.total_count > 0)
        {
            ...
            /* Setup fcu command */
            FACI->FSADDR = g_current_parameters.dest_addr;
            FACI_8->COM8 = (uint8_t) FCU_FACI_PROGRAM_CMD1;
            FACI_8->COM8 = (uint8_t) g_current_parameters.min_pgm_size;
        ...
        || (FLASH_CUR_CF_BGO_ERASE == g_current_parameters.current_operation))
    {
        ...
        if (g_current_parameters.current_count < g_current_parameters.total_count)
        {
            ...
            FACI->FSADDR = g_current_parameters.dest_addr;
            FACI_8->COM8 = (uint8_t) FCU_FACI_BLOCK_ERASE_CMD1;
            FACI_8->COM8 = (uint8_t) FCU_FACI_BLOCK_ERASE_CMD2;
