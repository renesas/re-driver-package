...
*         : 15.02.2018 0.50     First Release
*         : 02.10.2018 0.51     - Added setting to enable / disable bit modulation function
*                               - Changed to continue processing even if an error occurred in module 
*                                 un-stop release with the Uninitalize function
*                               - Changes not to cause errors even if TXI, RXI, ERI interrupts are not defined in 
*                                 the system
*                               - Update comments
*         : 17.12.2018 0.60     Added transmit / receive function by DMAC/DTC.
*                               Compatible with GCC compiler
*                               Adapted to GSCE coding rules.
*                               Changed file name to r_usart_cmsis_api.c
*                               Modified to comment out processing that does not pass when DMAC/DTC is not used.
*         : 22.01.2019 0.61     Fixed a bug that caused an error when calling the Receive function by specifying
*                               DMAC for transmission and DMAC other than DMAC.
*                               Fixed to clear the reception setting when DMAC Create of transmission
*                               failed by Transfer function.
*                               Delete DMA IRQ information from resource.
*                               Added processing to wait for the send buffer to become empty
*                               in Send / Receive / Transfer function when DTC and DMAC control is selected.
*         : 15.03.2019 0.70     Fixed to turn off the DMA driver definition when DMA is not used.
*         : 08.05.2019 0.72     Remove duplicates and unnecessary definitions.
*                               Add reception prohibition setting when overrun error occurs in smart card mode.
*         : 02.07.2019 1.00     Changed to use GetTxCount and GetRxCount when using DMA driver.
*                               Upgrade version to 1.00
*         : 28.01.2020 1.10     Fixed transmission permission processing
*                               Fixed a bug that interrupt does not occur when SCI9 is set to DTC control.
*         : 31.01.2020 1.20     Fixed transmission permission processing
...
/******************************************************************************
 Macro definitions
 *****************************************************************************/
#define ARM_USART_PRV_DRV_VERSION    (ARM_DRIVER_VERSION_MAJOR_MINOR(1, 40))  /*!< driver version */
