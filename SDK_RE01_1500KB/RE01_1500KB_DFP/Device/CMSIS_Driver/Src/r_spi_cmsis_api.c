...
*         : 04.10.2018 0.51     - Changed to perform send complete processing in slave mode with SPTEND interrupt.
*                               - Changes not to cause errors even if SPTI, SPRI, SPII, SPEI, SPTEND interrupts
*                                 are not defined in the system
*                               - Update comments
*         : 17.12.2018 0.60     Change the definition for timeout to SYSTEM_CFG_API_TIMEOUT_COUNT.
*                               Compatible with GCC compiler
*                               Added transmit / receive function by DMAC/DTC.
*                               Adapted to GSCE coding rules.
*                               Changed file name to r_spi_cmsis_api.c
*                               Modified to comment out processing that does not pass when DMAC/DTC is not used.
*         : 14.02.2019 0.61     Fix to access SPI data register of 8 bit size with SPI.SPDR_HH register.
*         : 14.02.2019 0.70     Fixed to turn off the DMA driver definition when DMA is not used.
*         : 26.04.2019 0.72     Fixed to close DMA even if resource lock fails in ARM_SPI_Initialize ()
*                               Fixed data bit length error return value from ARM_SPI_ERROR_BIT_ORDER to
*                               ARM_SPI_ERROR_DATA_BITS.
*                               Fixed RAM allocation judgment of DATA_LOCATION_PRV_DRIVER_SPI.
*                               Fixed to turn off the DMA driver definition when DMA is not used.
*         : 02.07.2019 1.00     Changed to use GetDataCount when using DMA driver.
*                               Upgrade version to 1.00
*         : 02.27.2020 1.10     Fixed bug where MSB/LSB settings were reversed.
*                               Upgrade version to 1.10
...
#define ARM_SPI_PRV_DRV_VERSION         (ARM_DRIVER_VERSION_MAJOR_MINOR(1, 40)) /*!< driver version */
...
static void r_spri_handler(st_spi_resources_t * const p_spi)
{
        ...
        (p_spi->xfer->rx_cnt < p_spi->xfer->num))
    {
        ...
        {
            ((uint8_t*)(p_spi->xfer->rx_buf))[p_spi->xfer->rx_cnt] = p_spi->reg->SPDR_HH;
        }
        ...
        (p_spi->xfer->tx_cnt < p_spi->xfer->num))
    {
        if (SPI_PRV_EXEC_RECEIVE == p_spi->xfer->exec_state)
        {
            ...
            else
            {
                ...
                p_spi->reg->SPDR_HH = (uint8_t)p_spi->xfer->tx_def_val;
            }
        }
        else
        {
            ...
            {
                /* SPDR - SPI Data Register - Depends on variables(8bit) */
                p_spi->reg->SPDR_HH = ((uint8_t*)(p_spi->xfer->tx_buf))[p_spi->xfer->tx_cnt];
            }
        }
        p_spi->xfer->tx_cnt++;
    }
