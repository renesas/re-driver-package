...
*         : 27.06.2019 0.72    Fix a bug. The return value of some functions will be overridden 
*                              and the return value will be wrong.
*                              - R_PMIP_Close
*                              - R_PMIP_PowerOn
*                              - R_PMIP_PowerOff
*                              - R_PMIP_Control
...
*                              - R_PMIP_AllOne
*                              - e_mlcd_allset
*         : 01.07.2019 0.72    Modify code to add interrupt service routine  
*         : 23.07.2019 1.00    Upgrade version to 1.00
*         : 06.08.2019 1.00    Fix a bug. The TEMI total counts is bigger than lines of image to transmission.
*         : 21.11.2019 1.01    Fixed the problem that the following internal functions are not placed in RAM
*                              even if they are set in RAM.
*                              - e_mlcd_cpu_normal_trans function
*         : 18.02.2020 1.10    Fixed RAM to place gs_pmip_resources variable to .ramdata
*                              Modified the wait time of R_PMIP_PowerOff function from 1ms to 2ms.
*                              VCOM output inhibit wait time changed from 4ms to 5ms
...
*                              Replace asm("nop"); with __NOP();
*         : 11.01.2021 1.40    Add R_PMIP_Reconfig().
**********************************************************************************************************************/
/******************************************************************************************************************//**
                             ...
                             uint8_t const data[])
{
    ...
    /* wait 5ms or more (Includes 1ms until VCOM goes low) */
    R_SYS_SoftwareDelay(5,SYSTEM_DELAY_UNITS_MILLISECONDS);
...
static void v_mlcd_clear_reg(void)
{
    ...
    gs_pmip_resources.reg_mlcd->MLCDSR      = 0x00000003;
    ...
    gs_pmip_resources.reg_mlcd->MLCDVCOMCTL = 0x08030000;
