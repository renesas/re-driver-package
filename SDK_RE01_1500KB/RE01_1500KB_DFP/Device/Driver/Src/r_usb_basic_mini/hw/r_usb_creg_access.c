 ...
 ******************************************************************************/
static void *hw_usb_get_fifosel_adr (uint16_t pipemode) FUNC_LOCATION_PRV_DRIVER_USB;
static void *hw_usb_get_fifoctr_adr (uint16_t pipemode) FUNC_LOCATION_PRV_DRIVER_USB;
...
{
    return (uint16_t)(USBFS->DVSTCTR0);
...
static void *hw_usb_get_fifosel_adr (uint16_t pipemode)
{
    ...
    {
        case USB_CUSE:
            p_reg = (void *)&(USBFS->CFIFOSEL);
        break;
        case USB_D0USE:
            p_reg = (void *)&(USBFS->D0FIFOSEL);
        break;
        case USB_D1USE:
            p_reg = (void *)&(USBFS->D1FIFOSEL);
        break;
        default:
...
static void *hw_usb_get_fifoctr_adr (uint16_t pipemode)
{
    ...
    {
        case USB_CUSE:
            p_reg = (void *)&(USBFS->CFIFOCTR);

        break;

        case    USB_D0USE:
            p_reg = (void *)&(USBFS->D0FIFOCTR);

            break;

        case    USB_D1USE:
            p_reg = (void *)&(USBFS->D1FIFOCTR);
...
{
    USBFS->INTSTS0 = (uint16_t) ~USB_SOFR;
...
{
    USBFS->BRDYSTS = (uint16_t) (~(1 << pipeno)) & USB_BRDYSTS_MASK;
...
{
    USBFS->NRDYSTS = (uint16_t) ((~(1 << pipeno)) & USB_NRDYSTS_MASK);
...
{
    USBFS->BEMPSTS = (uint16_t) ((~(1 << pipeno)) & USB_BEMPSTS_MASK);
...
{
    return (uint16_t)USBFS->PIPEMAXP;
...
uint16_t hw_usb_read_pipectr (uint16_t pipeno)
{
    ...
    if (USB_PIPE0 == pipeno)
    {
        p_reg = (uint16_t *)&(USBFS->DCPCTR);
    }
    else
    {
        p_reg = (uint16_t *)&(USBFS->PIPE1CTR) + (pipeno - 1);
    }
    return *p_reg;
...
void hw_usb_set_trenb (uint16_t pipeno)
{
    ...
    p_reg = (uint16_t *)&(USBFS->PIPE1TRE) + ((pipeno - 1) * 2);
    (*p_reg) |= USB_TRENB;
...
void hw_usb_write_pipetrn (uint16_t pipeno, uint16_t data)
{
    ...
    p_reg = (uint16_t *)&(USBFS->PIPE1TRN) + ((pipeno - 1) * 2);
    *p_reg = data;
