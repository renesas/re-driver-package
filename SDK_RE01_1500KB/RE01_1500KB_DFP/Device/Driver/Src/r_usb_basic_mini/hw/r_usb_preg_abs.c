...
void usb_pstd_interrupt_handler(uint16_t * p_type, uint16_t * p_status)
{
    ...
    /* Register Save */
    intsts0 = USBFS->INTSTS0;
    brdysts = USBFS->BRDYSTS;
    nrdysts = USBFS->NRDYSTS;
    bempsts = USBFS->BEMPSTS;
    intenb0 = USBFS->INTENB0;
    brdyenb = USBFS->BRDYENB;
    nrdyenb = USBFS->NRDYENB;
    bempenb = USBFS->BEMPENB;
    ...
    *p_status = 0;
    ...
    {
        /* Status clear */
        USBFS->INTSTS0 = (uint16_t)~USB_VBINT;
    ...
    {
        /* DVST clear */
        USBFS->INTSTS0 = (uint16_t)~USB_DVST;
    ...
    {
        USBFS->BRDYSTS = (uint16_t) ((~USB_BRDY0) & USB_BRDYSTS_MASK);
    ...
    {
        USBFS->BEMPSTS = (uint16_t) ((~USB_BEMP0) & USB_BEMPSTS_MASK);
        *p_type = USB_INT_BEMP;
        *p_status = USB_BEMP0;
    }
    else if ((USB_NRDY == (ists0 & USB_NRDY)) && (USB_NRDY0 == (nsts & USB_NRDY0)))
    {
        USBFS->NRDYSTS = (uint16_t) ((~USB_NRDY0) & USB_NRDYSTS_MASK);
        *p_type = USB_INT_NRDY;
        *p_status = USB_NRDY0;
    }
    ...
    else if (USB_CTRT == (ists0 & USB_CTRT))
    {
        ...
        *p_status = hw_usb_read_intsts();
        ...
        /* USB_CTRT clear */
        USBFS->INTSTS0 = (uint16_t)~USB_CTRT;
        *p_type = USB_INT_CTRT;
    ...
    {
        USBFS->BRDYSTS = (uint16_t) ((~bsts) & USB_BRDYSTS_MASK);
        *p_type = USB_INT_BRDY;
        *p_status = bsts;
    }
    else if (USB_BEMP == (ists0 & USB_BEMP))
    {
        USBFS->BEMPSTS = (uint16_t) ((~ests) & USB_BEMPSTS_MASK);
        *p_type = USB_INT_BEMP;
        *p_status = ests;
    }
    else if (USB_NRDY == (ists0 & USB_NRDY))
    {
        USBFS->NRDYSTS = (uint16_t) ((~nsts) & USB_NRDYSTS_MASK);
        *p_type = USB_INT_NRDY;
        *p_status = nsts;
    }
    else
...
void usb_pstd_forced_termination(uint16_t pipe, uint16_t status)
{
    ...
    usb_cstd_chg_curpipe((uint16_t)USB_PIPE0, (uint16_t)USB_CUSE, USB_NOUSE);
...
} /* End of function usb_pstd_select_nak */
