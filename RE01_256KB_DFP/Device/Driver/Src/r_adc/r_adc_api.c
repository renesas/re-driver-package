...
Macro definitions
**********************************************************************************************************************/
#define ADC_PRV_CHAN_LOW_MASK       (0x00FF)                          /*!< ADC register channel lower mask */
#define ADC_PRV_CHAN_HIGH_MASK      (0x08033)                         /*!< ADC register channel upper mask */
...
typedef enum e_adc_sst // @suppress("Source line ordering")
{
    ...
    AD_SST_AN007,                   /*!< Set the sampling state of AN007 */
    AD_SST_AN016_AN017_AN020_AN021_VSC_VCC,          /*!< Set the sampling state of AN016, AN017, AN020, AN021 and VSC_VCC */
...
{
    (ADC140_Type*)ADC140, // @suppress("Cast comment")
    R_S14AD_Pinset,
    R_S14AD_Pinclr,
    &gs_adc0_info,
    &gs_dma_info,
                     ...
                     st_adc_resources_t const * const p_adc)
{
    ...
    /* ADSSTRn - The ADSSTRn register sets the sampling time for analog input.(n = 0 ~ 6, L, T)
                 ...
                 ADSSTR6 - AN006
                 ADSSTR7 - AN007
                 ADSSTRL - AN016, AN017, AN020, AN021, VSC_VCC
    ...
    p_adc->reg->ADSSTR7 = default_sampling_rate;
    ...
    switch (group)
    {
        ...
        case ADC_GROUP_A:
        {
            ...
            /* ADANSA1   - A/D Channel Select Register A1
                           The ANSA16 bit corresponds to AN016, and the ANSA31 bit corresponds to VSC_VCC. */
        ...
        case ADC_GROUP_B:
        {
            ...
            /* ADANSB1   - A/D Channel Select Register B1
                           The ANSB16 bit corresponds to AN016, and the ANSB31 bit corresponds to VSC_VCC. */
        ...
        case ADC_GROUP_C:
        {
            ...
            /* ADANSC1   - A/D Channel Select Register C1
                           The ANSC16 bit corresponds to AN016, and the ANSC31 bit corresponds to VSC_VCC. */
                         ...
                         st_adc_resources_t const * const p_adc)
{
    ...
    switch (cmd)
    {
        ...
        case AD_CMD_SET_SAMPLING_AN007:             /* Change the sampling state of AN007                */
        {
            result = adc_cmd_sampling(AD_SST_AN007, (uint8_t*)p_args, p_adc); // @suppress("Cast comment")
        }
        break;

        case AD_CMD_SET_SAMPLING_AN016_AN017_AN020_AN021_VSC_VCC:/* Change the sampling state of AN016 , AN017, AN20, AN21 and VSC_VCC   */
        {
            result = adc_cmd_sampling(AD_SST_AN016_AN017_AN020_AN021_VSC_VCC, (uint8_t*)p_args, p_adc); // @suppress("Cast comment")
        }
        break;
                     ...
                     st_adc_resources_t const * const p_adc)
{
    ...
    switch (sel_ch)
    {
        ...
        case ADC_SSEL_VSC_VCC:
        {
            p_dregs = (uint16_t*)(&(p_adc->reg->ADVSCDR)); // @suppress("Cast comment")
        }
        break;
...
static e_adc_err_t adc_cmd_dbltrg(int8_t const * const p_dbltrg_ch, st_adc_resources_t * const p_adc)
{
    ...
    /* Returns an error if other than AN000 to AN007, AN016, AN017, AN020, AN021
       An error also occurs when 31 (VSC_VCC) is set. */
    if (((*p_dbltrg_ch) < (-1))
     || ((((*p_dbltrg_ch) > 7) && ((*p_dbltrg_ch) < 16)))
     || (18 == (*p_dbltrg_ch))
     || (19 == (*p_dbltrg_ch))
     || ((*p_dbltrg_ch) > 21))
    {
...
static e_adc_err_t adc_cmd_sampling(e_adc_sst_t sst ,uint8_t const * const p_sst_val, st_adc_resources_t * const p_adc)
{
    ...
    switch(sst)
    {
        ...
        case AD_SST_AN007:
        {
            p_adc->reg->ADSSTR7 = *p_sst_val;
        }
        break;
        
        case AD_SST_AN016_AN017_AN020_AN021_VSC_VCC:
        {
        ...
        case ADC_COMB_AND:
        {
            ...
            switch(p_winb->channel)
            {
                ...
                default:
                {
                    ...
                    if (p_winb->channel < 8)
                    {
                        /* Check AN000 to AD007 */
                        if (0 != (p_adc->reg->ADCMPANSR0 & (1 << p_winb->channel)))
                        {
                            return (ADC_ERROR_MODE);
                        }
                    }
                    else if ((p_winb->channel > 15) && (p_winb->channel < 32))
                    {
                        /* Check AN016 to AD021, and VSC_VCC */
...
static e_adc_err_t adc_cmd_calibration(st_adc_resources_t * const p_adc)
{
    ...
    /* ADC - Addition Count Select - 16-time conversion */
    p_adc->reg->ADADC_b.ADC = 5;
