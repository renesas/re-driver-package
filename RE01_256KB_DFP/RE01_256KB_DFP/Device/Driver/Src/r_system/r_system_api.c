 ...
 *         : 03.03.2020 0.80     Added the transition in the R_SYS_BoostSpeedModeSet()
 *                               from Low-leakage-current(VBB) mode to Boost mode.
 *                               Modified the transition Low-leakage-current(VBB) mode
 *                               to Normal-High speed mode.
 *                                - R_SYS_HighSpeedModeSet()
 *                               Modified the transition Low-leakage-current(VBB) mode
 *                               to Normal-Low speed mode.
 *                                - R_SYS_LowSpeedModeSet()
 *                               Added the value to gs_prcr_masks[] for PRC4 bit.
 *                               Added the FLL function operation.
 *                                - R_SYS_HighSpeedClockStart()
 *                               Added the Parameter Checking about FLL function.
 *                                - R_SYS_HighSpeedClockStart()
 ...
 *         : 24.11.2020 1.30     Deleted the comments as the FLL test has completed.
 **********************************************************************************************************************/
...
/*******************************************************************************************************************//**
 * @brief The module shall support the BOOST speed mode.
 * @param[in] None
 * @param[out] None
 * @retval 0 Success
 * @retval -1 Error
...
int32_t R_SYS_HighSpeedModeSet(void) /* @suppress("Function length") */
{
    ...
    switch (R_SYS_SpeedModeGet())
    {
        case SYSTEM_SPEED_MODE_BOOST:
        {
            /* Returns an error because mode is on the Low-speed mode after WFE instruction. */
            if(0x00U != SYSC->OPCCR_b.OPCM)
            {
                return -1;
            }
        ...
        case SYSTEM_SPEED_MODE_32KHZ:
        {
            ...
            /* Wait the transition from Low-leakage-current(VBB) mode to Normal mode. */
            while ((0U != SYSC->PWSTF_b.BOOSTM) || (0U != SYSC->PWSTF_b.VBBM))
            {
                __WFE();
            }
...
int32_t R_SYS_LowSpeedModeSet(void) /* @suppress("Function length") */
{
    ...
    /* It return an error if current power mode is ALLPWONM mode when entering into Low-Speed-mode. */
    if (R_LPM_PowerSupplyModeGet() == LPM_POWER_SUPPLY_MODE_ALLPWON)
    {
        return -1;
    }
    ...
    switch (R_SYS_SpeedModeGet())
    {
        case SYSTEM_SPEED_MODE_HIGH:
        {
            ...
            SYSC->OPCCR_b.OPCM = 3U;
            
            R_SYSTEM_PRV_REGISTER_PROTECT_ENABLE(SYSTEM_REG_PROTECT_OM_LPC_BATT);
            
            /* Wait the transition to Normal Low-Speed mode. */
            while (0x00U != SYSC->OPCCR_b.OPCMTSF)
            {
                ;   /* wait */
            }
        }
        ...
        case SYSTEM_SPEED_MODE_32KHZ:
        {
            /* Returns an error because mode is on the High-speed mode after WFE instruction. */
            if (0x03U != SYSC->OPCCR_b.OPCM)
            {
                return -1;
            }
...
int32_t R_SYS_SystemClockHOCOSet(void) /* @suppress("Function length") */
{
    ...
    /* The module checks HOCOCR register if HOCO operation.
       ...
       If not, return an error. */
    if ((1U == SYSC->HOCOCR_b.HCSTP) || (0U == SYSC->OSCSF_b.HOCOSF))
    {
        return -1;
    }
    ...
    if ((SYSTEM_CFG_HOCO_FREQUENCY > 1) && (SYSTEM_CLOCK_DIV_1 == SYSC->SCKDIVCR_b.PCKB))
    {
        return -1;
    }
    ...
    if (R_SYS_SystemClockFreqGet(&freq_hz) == 0)
    {
        if (32000000U >= freq_hz)
        {
            ...
            if (32000000U < (R_SYSTEM_PRV_HOCO_FREQUENCY_HZ / (1 << SYSC->SCKDIVCR_b.ICK)))
            {
                /* In case of "Boost mode". */
                if (1U == SYSC->PWSTF_b.BOOSTM)
                {
                    ...
                    SYSC->SCKSCR = R_SYSTEM_PRV_SCKSCR_CKSEL_HOCO;
...
int32_t R_SYS_SystemClockMOCOSet(void)
{
    ...
    if (1U == SYSC->PWSTF_b.VBBM)
    {
        /* Must be in "NORMAL OPE" or "BOOST OPE". */
        return -1;
    }
#endif /* (SYSTEM_CFG_PARAM_CHECKING_ENABLE == 1) */
    ...
    /* Set the system clock source. */
    SYSC->SCKSCR = R_SYSTEM_PRV_SCKSCR_CKSEL_MOCO;
...
int32_t R_SYS_SystemClockLOCOSet(void)
{
    ...
    /* Set the system clock source. */
    SYSC->SCKSCR = R_SYSTEM_PRV_SCKSCR_CKSEL_LOCO;
...
int32_t R_SYS_SystemClockMOSCSet(void)
{
    ...
    /* The module checks MOSCCR register if main clock operation.
       ...
       If not, return an error. */
    if ((1U == SYSC->MOSCCR_b.MOSTP) || (0U == SYSC->OSCSF_b.MOSCSF))
    {
        /* It is necessary to call R_SYS_MainOscSpeedClockStart(). */
        return -1;
    }
    ...
    /* Set the system clock source. */
    SYSC->SCKSCR = R_SYSTEM_PRV_SCKSCR_CKSEL_MOSC;
...
int32_t R_SYS_SystemClockSOSCSet(void)
{
    ...
    /* Set the system clock source. */
    SYSC->SCKSCR = R_SYSTEM_PRV_SCKSCR_CKSEL_SOSC;
...
int32_t R_SYS_SystemClockFreqGet(uint32_t * p_freq_hz)
{
    ...
    switch (SYSC->SCKSCR & R_SYSTEM_PRV_SCKSCR_CKSEL_MSK)
    {
        case R_SYSTEM_PRV_SCKSCR_CKSEL_HOCO:
        {
            /* Get the ICLK/PCLKA frequency. */
            *p_freq_hz = (R_SYSTEM_PRV_HOCO_FREQUENCY_HZ / (1 << SYSC->SCKDIVCR_b.ICK));
        }
        break;
        case R_SYSTEM_PRV_SCKSCR_CKSEL_MOCO:
        {
            /* Get the ICLK/PCLKA frequency. */
            *p_freq_hz = (R_SYSTEM_PRV_MOCO_FREQUENCY_HZ / (1 << SYSC->SCKDIVCR_b.ICK));
        }
        break;
        case R_SYSTEM_PRV_SCKSCR_CKSEL_LOCO:
        {
            /* Get the ICLK/PCLKA frequency. */
            *p_freq_hz = (R_SYSTEM_PRV_LOCO_FREQUENCY_HZ / (1 << SYSC->SCKDIVCR_b.ICK));
        }
        break;
        case R_SYSTEM_PRV_SCKSCR_CKSEL_MOSC:
        {
            /* Get the ICLK/PCLKA frequency. */
            *p_freq_hz = (SYSTEM_CFG_MOSC_FREQUENCY_HZ / (1 << SYSC->SCKDIVCR_b.ICK));
        }
        break;
        case R_SYSTEM_PRV_SCKSCR_CKSEL_SOSC:
        {
            /* Get the ICLK/PCLKA frequency. */
            *p_freq_hz = (R_SYSTEM_PRV_SUBCLOCK_FREQUENCY_HZ / (1 << SYSC->SCKDIVCR_b.ICK));
        }
        break;
        default:
        {
...
int32_t R_SYS_PeripheralClockFreqGet(uint32_t * p_freq_hz)
{
    ...
    switch (SYSC->SCKSCR & R_SYSTEM_PRV_SCKSCR_CKSEL_MSK)
    {
        case R_SYSTEM_PRV_SCKSCR_CKSEL_HOCO:
        {
            ...
            *p_freq_hz = (R_SYSTEM_PRV_HOCO_FREQUENCY_HZ / (1 << SYSC->SCKDIVCR_b.PCKB));
        }
        break;
        case R_SYSTEM_PRV_SCKSCR_CKSEL_MOCO:
        {
            /* Get the PCLKB frequency. */
            *p_freq_hz = (R_SYSTEM_PRV_MOCO_FREQUENCY_HZ / (1 << SYSC->SCKDIVCR_b.PCKB));
        }
        break;
        case R_SYSTEM_PRV_SCKSCR_CKSEL_LOCO:
        {
            /* Get the PCLKB frequency. */
            *p_freq_hz = (R_SYSTEM_PRV_LOCO_FREQUENCY_HZ / (1 << SYSC->SCKDIVCR_b.PCKB));
        }
        break;
        case R_SYSTEM_PRV_SCKSCR_CKSEL_MOSC:
        {
            /* Get the PCLKB frequency. */
            *p_freq_hz = (SYSTEM_CFG_MOSC_FREQUENCY_HZ / (1 << SYSC->SCKDIVCR_b.PCKB));
        }
        break;
        case R_SYSTEM_PRV_SCKSCR_CKSEL_SOSC:
        {
            /* Get the PCLKB frequency. */
            *p_freq_hz = (R_SYSTEM_PRV_SUBCLOCK_FREQUENCY_HZ / (1 << SYSC->SCKDIVCR_b.PCKB));
        }
        break;
        default:
        {
...
int32_t R_SYS_SystemClockDividerSet(e_system_sys_clock_div_t iclk_div, e_system_sys_clock_div_t pclkb_div) /* @suppress("Function length") */
{
    ...
    if ((SYSTEM_CLOCK_NONE_CHANGE == iclk_div) && (pclkb_div < SYSC->SCKDIVCR_b.ICK))
    {
        return -1;
    }
    ...
    if (SYSTEM_CLOCK_NONE_CHANGE == pclkb_div)
    {
        ...
        if ((SYSTEM_CLOCK_NONE_CHANGE != iclk_div) && (iclk_div > SYSC->SCKDIVCR_b.PCKB))
        {
            return -1;
        }
    }
    ...
    if ((iclk_div != SYSC->SCKDIVCR_b.ICK) && (SYSTEM_CLOCK_NONE_CHANGE == iclk_div))
    {
        /* Get the current setting divider. */
        iclk_div_set = SYSC->SCKDIVCR_b.ICK;
    }
    
    /* In case of the argument is different from the current setting divider. */
    if ((pclkb_div != SYSC->SCKDIVCR_b.PCKB) && (SYSTEM_CLOCK_NONE_CHANGE == pclkb_div))
    {
        /* Get the current setting divider. */
        pclkb_div_set = SYSC->SCKDIVCR_b.PCKB;
    }
    
    /* In case of arguments are same as the current setting dividers. */
    if ((SYSC->SCKDIVCR_b.ICK == iclk_div_set) && (SYSC->SCKDIVCR_b.PCKB == pclkb_div_set))
    {
        /* Do Nothing. */
        ;
    }
    else
    {
         ...
         *------------------------------------------------------------------------*/
        SYSC->SCKDIVCR = (uint32_t)(( iclk_div_set << 0x18U) |
                                      (pclkb_div_set << 0x08U));
...
void R_SYS_MainOscSpeedClockStart(void)
{
    ...
    if ((1U == SYSC->MOSCCR_b.MOSTP) && (0U == SYSC->OSCSF_b.MOSCSF))
    {
        ...
        SYSC->MOSCCR_b.MOSTP = 0U;
        
        R_SYSTEM_PRV_REGISTER_PROTECT_ENABLE(SYSTEM_REG_PROTECT_CGC);
        
        /* Wait until the register value has changed. */
        while(0U != SYSC->MOSCCR_b.MOSTP)
        {
            ;   /* wait */
        }
...
int32_t R_SYS_MainOscSpeedClockStop(void)
{
    ...
    /* If the system clock source is main clock oscillation, do not stop the main clock. */
    if (R_SYSTEM_PRV_SCKSCR_CKSEL_MOSC == SYSC->SCKSCR_b.CKSEL)
    {
    ...
    if (0U == SYSC->MOSCCR_b.MOSTP)
    {
        ...
        /* Start the main clock oscillator operation. */
        SYSC->MOSCCR_b.MOSTP = 1U;
        
        R_SYSTEM_PRV_REGISTER_PROTECT_ENABLE(SYSTEM_REG_PROTECT_CGC);
        
        /* Wait until the register value has changed. */
        while(1U != SYSC->MOSCCR_b.MOSTP)
        {
            ;   /* wait */
        }
...
int32_t R_SYS_HighSpeedClockStart(void)
{
    ...
    if ((0U == SYSC->PWSTF_b.BOOSTM) && (1U < SYSTEM_CFG_HOCO_FREQUENCY))
    {
        return -1;
    }
    ...
    /* If the SOSC is stop, FLL function cannot be enable. */
    if ((1U == SYSTEM_CFG_FLL_ENABLE) && (0U != SYSC->SOSCCR_b.SOSTP))
    {
        return -1;
    }
...
int32_t R_SYS_HighSpeedClockStop(void)
{
    ...
    if ((0U == SYSC->HOCOCR_b.HCSTP) && (R_SYSTEM_PRV_SCKSCR_CKSEL_HOCO == SYSC->SCKSCR_b.CKSEL))
    {
        return -1;
    }
...
int32_t R_SYS_MediumSpeedClockStart(void)
{
    ...
    if(1U == SYSC->MOCOCR_b.MCSTP)
    {
         ...
         * b7-b1 : [Reserved] Set the 0
         *--------------------------------------------------------------------*/
        SYSC->MOCOCR_b.MCSTP = 0U;
        
        R_SYSTEM_PRV_REGISTER_PROTECT_ENABLE(SYSTEM_REG_PROTECT_CGC);
        
        /* Wait until the register value has changed. */
        while(0U != SYSC->MOCOCR_b.MCSTP)
        {
            ;   /* wait */
        }
    }
...
int32_t R_SYS_MediumSpeedClockStop(void)
{
    ...
    if (R_SYSTEM_PRV_SCKSCR_CKSEL_MOCO == SYSC->SCKSCR_b.CKSEL)
    {
        return -1;
    }
    ...
    if(0U == SYSC->MOCOCR_b.MCSTP)
    {
         ...
         * b7-b1 : [Reserved] Set the 0
         *--------------------------------------------------------------------*/
        SYSC->MOCOCR_b.MCSTP = 1U;
        
        R_SYSTEM_PRV_REGISTER_PROTECT_ENABLE(SYSTEM_REG_PROTECT_CGC);
        
        /* Wait until the register value has changed. */
        while(1U != SYSC->MOCOCR_b.MCSTP)
        {
            ;   /* wait */
        }
    }
...
int32_t R_SYS_LowSpeedClockStop(void)
{
    ...
    if (R_SYSTEM_PRV_SCKSCR_CKSEL_LOCO == SYSC->SCKSCR_b.CKSEL)
    {
        return -1;
    }
    
    /* In VBB mode and VBB clock source is LOCO, do not stop LOCO. */
    if ((1U == SYSC->VBBCR_b.VBBEN) && (0U == SYSC->VBBCR_b.CLKSEL))
    {
        return -1;
    }
    ...
    if(0U == SYSC->LOCOCR_b.LCSTP)
    {
         ...
         * b7-b1 : [Reserved] Set the 0
         *--------------------------------------------------------------------*/
        SYSC->LOCOCR_b.LCSTP = 1U;
        
        R_SYSTEM_PRV_REGISTER_PROTECT_ENABLE(SYSTEM_REG_PROTECT_CGC);
        
        /* Wait until the register value has changed. */
        while(1U != SYSC->LOCOCR_b.LCSTP)
        {
            ;   /* wait */
        }
    }
...
void R_SYS_SubOscSpeedClockStart(void)
{
    ...
    if(1U == SYSC->SOSCCR_b.SOSTP)
    {
         ...
         * b7-b1 : [Reserved] Set the 0
         *--------------------------------------------------------------------*/
        SYSC->SOSCCR_b.SOSTP = 0U;
        
        R_SYSTEM_PRV_REGISTER_PROTECT_ENABLE(SYSTEM_REG_PROTECT_CGC);
        
        /* Wait until the register value has changed. */
        while(0U != SYSC->SOSCCR_b.SOSTP)
        {
            ;   /* wait */
        }
    }
...
int32_t R_SYS_SubOscSpeedClockStop(void)
{
    ...
    if (R_SYSTEM_PRV_SCKSCR_CKSEL_SOSC == SYSC->SCKSCR_b.CKSEL)
    {
        return -1;
    }
    
    /* If it is in VBB OPE and selected SOSC, do not stop SOSC. */
    if ((1U == SYSC->VBBCR_b.VBBEN) && (1U == SYSC->VBBCR_b.CLKSEL))
    {
        return -1;
    }
    
    /* If the FLL function is enable, do not stop SOSC. */
    if ((1U == SYSC->FLLCR1_b.FLLEN) && (0U == SYSC->HOCOCR_b.HCSTP))
    {
        return -1;
    }
    ...
    if(0U == SYSC->SOSCCR_b.SOSTP)
    {
         ...
         * b7-b1 : [Reserved] Set the 0
         *--------------------------------------------------------------------*/
        SYSC->SOSCCR_b.SOSTP = 1U;
        
        R_SYSTEM_PRV_REGISTER_PROTECT_ENABLE(SYSTEM_REG_PROTECT_CGC);
        
        /* Wait until the register value has changed. */
        while(1U != SYSC->SOSCCR_b.SOSTP)
        {
            ;   /* wait */
        }
    }
 ...
 *        If wait the main clock oscillation or HOCO clock oscillation,
